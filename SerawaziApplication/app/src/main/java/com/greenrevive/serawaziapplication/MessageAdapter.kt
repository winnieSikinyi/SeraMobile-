...
class MessageAdapter(private val messageList: MutableList<com.greenrevive.serawaziapplication.model.Message>) :
    ...
    inner class MessageViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(message: com.greenrevive.serawaziapplication.model.Message) {
            when (message.sentBy) {
                Message.SENT_BY_ME -> {
                    rightChatTextView?.text = message.message
                    rightChatTimestamp?.text = message.timestamp.toString()
                }
                else -> {
                    leftChatTextView?.text = message.message
                    leftChatTimestamp?.text = message.timestamp.toString()
                }
            }
        }
    }
